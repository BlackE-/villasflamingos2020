    ...
    const setContainer = (data,container) =>{
        stringData = ``;
        stringData += `<div class="habitacion_text"><div class="amenitades_box capacidad"><p class="number">${data.capacity}</p><p class="title">Capacidad<br>máxima</p></div>`;
        stringData += `     <div class="amenitades_box cama"><img src="img/villas-icono-cama.svg" width="106" height="100"/><p class="title">${data.bed}</p></div>`;
        ...
        stringData += `     <div class="amenitades_box terrazaprivada"><img src="img/villas-icono-terraza.svg"  width="106" height="100"/><p class="title">Terraza Privada</p></div>`;
        stringData += `     <div class="amenitades_box superficie"><p class="number">${data.surface} m²</p><p class="title">Superficie</p></div>`;
        stringData += `     <div class="amenitades_box niveles"><p class="number">${data.level}</p><p class="title">Niveles</p></div>`;
        ...
        stringData += `      <div class="amenitades_box albertaprivada">`;
    ...
    const setImages = (data,container) =>{
        if(window.innerWidth < 400){/*add only one img*/document.getElementById(container).innerHTML = `<div class="imgBox"><amp-img alt="${data.room}" src="img/habitaciones-${data.room}-1-mobile.jpg"  width="360" height="240"  layout="responsive"></amp-img></div>`;
            ...
            stringData = ``;
            for(let x = 1;x<=data.num;x++){stringData += `<div class="imgBox"><amp-img class='active' alt="${data.room}" src="img/habitaciones-${data.room}-${x}.jpg"  width="1080" height="720"  layout="responsive"></amp-img></div>`;}
            document.getElementById(container).innerHTML = stringData;
        }
